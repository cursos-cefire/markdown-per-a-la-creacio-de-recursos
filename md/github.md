...
<figure markdown>
  ...
  <figcaption>URL per a la clonació de repositori buit</figcaption>
</figure>

<figure markdown>
  ![Clonar repositori buit](img/github/clonar3.png)
  <figcaption>URL per a la clonació de repositori amb contingut</figcaption>
</figure>
...
En sistemes Windows podeu visitar el següent [enllaç](https://github.com/git-for-windows/git/releases/) i baixar l'última versió (Git-2.51.1-64-bit.exe en el moment d'escriure estos apunts).

!!!info "Repositori clonat en local"
...
!!!warning "mkdocs new"
    ...
    Altra opció és, navegar a la carpeta del projecte github clonat i fer ús del `.` per a indicar que vols crear en el directori actual el projecte mkdocs:

    `mkdocs new .`

    Si tot ha anat bé, veuràs que es crea l'arxiu `mkdocs.yml` i la carpeta `docs` al repositori.
...
!!!important "Carpeta docs i site"
    ...
    Per solucionar-ho tenim duess possibles solucions:

    1. Executar el `mkdocs build` i copiar el contingut de la carpeta `site` a l'arrel del repositori (opció recomanada si es volen mantindre privat els arxius font).
    2. Modificar la configuració per a que els arxius font en markdown estiguen en una altra carpeta i que el resultat del build estiga a la carpeta docs (opció recomanada quan es vol tindre **en un mateix repositori de github els fonts en markdown i la web construida**).


Explicarem la segona opció. Simplement hem d'indicar a l'arxiu de configuració de mkdocs que els arxius font en format markdown estan en la carpeta `md` i que la construcció es guardarà a la carpeta `docs`:
...
!!!warning "Configuració de github per a traballar des de VSCode"

    Per a evitar que ens fallen els commits, anem a configurar el nostre nom i el nostre correu a la configuració de Git, amb les següents dues ordres, introduint les vostres dades corresponents (el correu i el nom d'usuari són els mateixosa utilitzats durant el registre de github):
...
![Github Branch Folder](img/github/pages-branch-folder.png)

En breu vos indicarà si la pàgina ja està publicada o si per contra hi ha hagut algun error. 

Si tot ha anat bé us indicarà l'adreça on s'ha publicat, que serà **https://nom_usuari.github.io/nom_repositori**. Comproveu que ja podeu navegar per ella i es veu tal qual la veieu al servidor local.
